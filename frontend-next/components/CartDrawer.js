      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5" style={{ maxHeight: "calc(100vh - 260px)" }}>
          {items.length === 0 && (
            <div className="text-center py-16">
              ...
              <button data-testid="cart-empty-shop" onClick={() => { setOpen(false); router.push("/shop"); }} className="btn-primary mt-6">Browse Oils</button>
            </div>
          )}
          {items.map((i) => (
        ...
        {items.length > 0 && (
          <div className="border-t p-6 space-y-3" style={{ borderColor: "var(--line)" }}>
            ...
            <button data-testid="cart-checkout" onClick={() => { setOpen(false); router.push("/checkout"); }} className="btn-primary w-full justify-center">Checkout</button>
          </div>
        )}
      </aside>
    </>
